...
class TsuServerCC:
    ...
    def __init__(self):
        ...
        if os.path.exists('storage/db.sqlite3'):
            if (self.config['max_dbsize'] != 0) and (os.stat('storage/db.sqlite3').st_size > self.config['max_dbsize']):
                confirmation = input('!--- The database for this server is larger than the maximum size set in config. If you wish to clear stored logs to reduce size, type "yes".\n>')
                if confirmation.lower() == 'yes':
                    print(database.clear_logs())
        else:
